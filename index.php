        	...
        	<tr>
				...
				<div class="index-auth"  >
						<form method="POST" action="<?= $_SERVER['ORIG_PATH_INFO'] ?>" 
							name="auth" style="visibility: <?= $_SESSION['authCheck'] ? 'hidden' : 'visible' ?>"/><!-- выводит атрибут для селектора visibility -->
							<table  width="100%" border="0" cellspacing="0" cellpadding="0" >
								
								<tr style="visibility: <?= $loginInputField ?>">
									<td class="iat">Ваш e-mail: <br /> <input id="login_id" 
										value="<?= $inputLogin ?>" size="30" name="login" /></td>
								</tr>
								<tr>
									<td class="iat">Ваш пароль: <br /> <input type="password" id="password_id" value="<?= $inputPassword ?>" size="30" name="password" autocomplete="password"/>
									</td> 
								</tr>
								<tr>
						...
						<?  //выводит сообщения после отправки формы авторизации
						    ...
						    if (isset($_SESSION['authCheck'])) {
						        $_SESSION['authCheck'] ? require_once($_SERVER['DOCUMENT_ROOT'] . 
						        '/include/success.php') : 
